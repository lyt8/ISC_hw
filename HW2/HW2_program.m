...
[N_cu, all_err_cu, y_uniform_clamped] = findN(x_plot, y_plot, N, M, tol, 'c', 'u');
[N_nu, all_err_nu, y_uniform_natural] = findN(x_plot, y_plot, N, M, tol, 'n', 'u');
[N_cc, all_err_cc, y_cheby_clamped] = findN(x_plot, y_plot, N, M, tol, 'c', 'c');
[N_nc, all_err_nc, y_cheby_natural] = findN(x_plot, y_plot, N, M, tol, 'n', 'c');
...
function[N, all_err, y_cubic] = findN(x_plot, y_plot, N, M, tol, bcType, nodeType)
    while true
        ...
        % Check tolerance
        if err < tol
            fprintf('When N = %d, the error = %e and it is less than %e', N, err, tol);
            fprintf('\n')
            break;
        else
            N = N + 1; 
